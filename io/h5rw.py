...
def h5read(filename, *args, **kwargs):
    ...
    def _load_numpy_record_array(dset):
        return pickle.loads(dset.value.encode('utf-8'))
    ...
    def _load_pickle(dset):
        return pickle.loads(dset.value)
    ...
    def _load_non_h5rw(dset, depth, sl=None):
        if type(dset) is h5py.Group:
            # Load subgroups
            val = _load_dict(dset, depth)

            # Append all attributes as keys
            for k, v in list(dset.attrs.items()):
                if k == 'NX_class': continue
                val[k] = v
        else:
            # Only numpy arrays supported in this case
            val = _load_numpy(dset, sl)
        return val
