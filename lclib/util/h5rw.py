...
def _h5write(filename, mode, *args, **kwargs):
    ...
    _h5write(filename, mode, dict, compress=True, var1=..., var2=...)
    ...
    A boolean parameter named 'compress' can be passed inside kwargs.
    If True GZIP level 4 compression is used, if False compression is off.
    Default is True.
    ...
    * dictionaries
    ...
    is_open = (type(filename) is h5py.File)

    if is_open and mode != 'a':
        raise RuntimeError('Use h5append when writing to file handles.')
    ...
    if 'compress' in list(kwargs.keys()): # search for 'compress' in the kwargs
        compress = kwargs['compress']
        kwargs.pop('compress')
    else:
        compress = True # this is the default value
    d.update(kwargs) # append kwargs to the input dictionary
    ...
    # @sdebug
    def _store_numpy(group, a, name, compress):
        if compress:
        ...
        if arrayOK:
            try:
                ...
                if la.dtype.type is np.string_:
                    ...
                    dset = _store_numpy(group, la, name, compress)
    ...
    def _store(group, a, name):
        if type(a) is str:
            ...
            dset = _store_numpy(group, a, name, compress)
...
def h5write(filename, *args, **kwargs):
    ...
    h5write(filename, dict, compress=True, var1=..., var2=...)
